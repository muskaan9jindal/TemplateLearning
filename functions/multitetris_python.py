    ...
    for _ in trange(Iterations):
        ...
        for mol, freq in zip(mols, frequencies):
            ...
            for f in range(freq): ###ChroM
                ...
                if insersion_distances[1] == 0:
                    out_layer = binary_closing(rotated_binary, ball(1))
                elif insersion_distances[1] < 0:
                    out_layer = binary_closing(rotated_binary, ball(1))
                    out_layer = binary_erosion(out_layer, ball(-insersion_distances[1]))
                else:
                    out_layer = binary_dilation(rotated_binary, ball(insersion_distances[1]))

                if insersion_distances[0] == 0:
                    in_layer = binary_closing(rotated_binary, ball(1))
                elif insersion_distances[0] < 0:
                    in_layer = binary_closing(rotated_binary, ball(1))
                    in_layer = binary_erosion(in_layer, ball(-insersion_distances[0]))
                else:
